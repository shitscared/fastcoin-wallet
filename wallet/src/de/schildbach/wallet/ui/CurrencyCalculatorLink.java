...
{
	private final CurrencyAmountView fstAmountView;
	...
	private final CurrencyAmountView.Listener fstAmountViewListener = new CurrencyAmountView.Listener()
	{
		@Override
		public void changed()
		{
			if (fstAmountView.getAmount() != null)
	...
	public CurrencyCalculatorLink(@Nonnull final CurrencyAmountView fstAmountView, @Nonnull final CurrencyAmountView localAmountView)
	{
		this.fstAmountView = fstAmountView;
		this.fstAmountView.setListener(fstAmountViewListener);
	...
	public BigInteger getAmount()
	{
		...
		{
			return fstAmountView.getAmount();
		}
		else if (exchangeRate != null)
		{
			final BigInteger localAmount = localAmountView.getAmount();
			return localAmount != null ? WalletUtils.fstValue(localAmount, exchangeRate.rate) : null;
		}
		else
		{
			return null;
	...
	private void update()
	{
		fstAmountView.setEnabled(enabled);
		...
		if (exchangeRate != null)
		{
			...
			if (exchangeDirection)
			{
				final BigInteger fstAmount = fstAmountView.getAmount();
				if (fstAmount != null)
				{
					localAmountView.setAmount(null, false);
					localAmountView.setHint(WalletUtils.localValue(fstAmount, exchangeRate.rate));
					fstAmountView.setHint(null);
				}
			}
			else
			{
				...
				if (localAmount != null)
				{
					fstAmountView.setAmount(null, false);
					fstAmountView.setHint(WalletUtils.fstValue(localAmount, exchangeRate.rate));
					localAmountView.setHint(null);
				}
			}
		}
	...
	{
		if (exchangeDirection)
			return fstAmountView.getTextView();
		else
	...
	{
		fstAmountView.setAmount(amount, true);
	}

	public void setNextFocusId(final int nextFocusId)
	{
		fstAmountView.setNextFocusId(nextFocusId);
